...
from .utils import plot_hist, plot_distributions
...
class RegressorDiagnostic(ModelDiagnostic):
    ...
    def __init__(self, model, feature_name_list, target_name, data_train, data_test, output_name):
        ...
        self.target_estimation_name = self.output_name
        self.target_estimation_name_img = self.output_name + '_img'

        # Compute and add target estimation
        self.data_train = self.add_image_model_output(
            self.data_train,
            col_name=self.target_estimation_name_img
        )
...
class ClassifierDiagnostic(ModelDiagnostic):
    ...
    score_name: `str`
        Name of output
    """
    def __init__(self, model, feature_name_list, target_name, data_train, data_test, score_name='score'):
        super().__init__(model, feature_name_list, target_name)

        self.data_train = data_train
        self.data_test = data_test
        self.score_name = score_name
        self.score_img_name = score_name + '_img'

        # Compute and add score
        self.data_train = self.add_image_model_output(
            self.data_train,
            col_name=self.score_img_name
        )
        self.data_test = self.add_image_model_output(
            self.data_test,
            col_name=self.score_img_name
    ...
    def plot_image_score_distribution(
            self,
            nbin=30,
            hist_kwargs_list=[
        ...
        """Plot score distribution. Need score column"""
        return plot_distributions(
            [self.score_img_name],
    ...
    def plot_roc_curve(self, ax, **kwargs):
        fpr, tpr, _ = roc_curve(
            self.data_test['label'],
            self.data_test[self.score_img_name]
        )
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, label='ROC curve (area={:.2f})'.format(roc_auc), **kwargs)
        ax.set_xlabel('False Positive Rate')
        ax.set_ylabel('True Positive Rate')
        return ax

    def plot_evt_score_distribution(self,
                                    data_train,
                                    data_test,
                                    nbin=30,
                                    hist_kwargs_list=[
    ...
    def plot_evt_score_distribution_variation(self,
                                              data_train,
                                              data_test,
                                              cut_list,
                                              nbin=30,
        ...
        for i, colname in enumerate(cut_list):
            ...
            # Range for binning
            range_min = min([data[self.score_name].min() for data in data_list])
            range_max = max([data[self.score_name].max() for data in data_list])
            myrange = [range_min, range_max]

            for j, data in enumerate(data_list):
                ax = plot_hist(
                    ax=ax, data=data.query(cut_list[i])[self.score_name],
    ...
    def plot_evt_roc_curve_variation(self, ax, data_test, cut_list):
        ...
        for i, cut in enumerate(cut_list):
            ...
            fpr, tpr, _ = roc_curve(data['label'], data[self.score_name])
            roc_auc = auc(fpr, tpr)
            ax.plot(
                fpr, tpr, color=str(c), lw=2,
                label='{} (area={:.2f})'.format(cut.replace('reco_energy', 'E'), roc_auc)
            )
            ax.set_xlabel('False Positive Rate')
            ax.set_ylabel('True Positive Rate')
