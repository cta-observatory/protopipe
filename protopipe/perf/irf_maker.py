...
class SensitivityMaker(object):
    ...
    def estimate_sensitivity(self):
        obs_time = self.config['analysis']['obs_time']['value'] * \
                   u.Unit(self.config['analysis']['obs_time']['unit'])
    ...
    def add_sensitivity_to_irf(self):
        ...
        t = Table()
        t['ENERG_LO'] = Column(self.irf.bkg.energy.lo.value, unit='TeV', description='energy min', format='E')
        t['ENERG_HI'] = Column(self.irf.bkg.energy.hi.value, unit='TeV', description='energy max', format='E')
        t['SENSITIVITY'] = Column(self.sens['e2dnde'], unit='erg/(cm2 s)', description='sensitivity', format='E')
        t['EXCESS'] = Column(self.sens['excess'], unit='', description='excess', format='E')
        t['BKG'] = Column(self.sens['background'], unit='', description='bkg', format='E')

        filename = os.path.join(self.outdir, 'irf.fits.gz')
        hdulist = fits.open(filename, memmap=False, mode='update')
        col_list = [fits.Column(col.name, col.format,
                                unit=str(col.unit),
                                array=col.data) for col in t.columns.values()]
...
class IrfMaker(object):
    ...
    def __init__(self, config, evt_dict, outdir):
        ...
        # Binning
        self.ereco = np.logspace(np.log10(self.table['emin'][0]),
                                 np.log10(self.table['emax'][-1]),
                                 len(self.table))

        cfg_binning = config['analysis']['etrue_binning']
        self.etrue = np.logspace(np.log10(cfg_binning['emin']),
                                 np.log10(cfg_binning['emax']),
                                 cfg_binning['nbin'] + 1)
    ...
    def make_bkg_rate(self):
        ...
        for ibin, info in enumerate(self.table):
            ...
            n_p = sum(data_p[(data_p['reco_energy'] >= info['emin']) &
                             (data_p['reco_energy'] < info['emax']) &
                             (data_p['pass_best_cutoff'])]['weight'])

            n_e = sum(data_e[(data_e['reco_energy'] >= info['emin']) &
                             (data_e['reco_energy'] < info['emax']) &
                             (data_e['pass_best_cutoff'])]['weight'])

            # Correct number of background due to acceptance
            acceptance_g = 2 * np.pi * (1 - np.cos(info['angular_cut'] * u.deg.to('rad')))
            acceptance_p = 2 * np.pi * (1 - np.cos(self.config['particle_information']['proton']['offset_cut'] * u.deg.to('rad')))
            acceptance_e = 2 * np.pi * (1 - np.cos(self.config['particle_information']['electron']['offset_cut'] * u.deg.to('rad')))
        ...
        return IrfMaker._make_hdu(
            'BACKGROUND', t, ['ENERG_LO', 'ENERG_HI', 'BGD']
        )
    ...
    def make_point_spread_function(self, radius=68):
        ...
        for ibin, info in enumerate(self.table):
            ...
            sel = data_g.loc[(data_g['reco_energy'] >= info['emin']) &
                             (data_g['reco_energy'] < info['emax']) &
                             (data_g['pass_best_cutoff']), ['xi']]
    ...
    def make_energy_dispersion(self):
        ...
        # Select events
        data_g = self.evt_dict['gamma']
        data_g = data_g[(data_g['pass_best_cutoff']) & (data_g['pass_angular_cut'])].copy()
        ...
        table_migra['MIGRA_HI'] = Column(migra[1:], unit='', description='migra max', format=str(len(migra) - 1) + 'E')

        # Needed for format, a bit hacky...
        theta_lo = [0., 1.]
        theta_hi = [1., 2.]
        table_theta = Table()
        table_theta['THETA_LO'] = Column(theta_lo, unit='deg', description='theta min', format=str(len(theta_lo)) + 'E')
        table_theta['THETA_HI'] = Column(theta_hi, unit='deg', description='theta max', format=str(len(theta_hi)) + 'E')

        extended_mig_matrix = np.resize(counts, (len(theta_lo), counts.shape[0], counts.shape[1]))
        dim_matrix = len(table_energy['ETRUE_LO']) * len(table_migra['MIGRA_LO']) * len(table_theta['THETA_LO'])
    ...
    def _make_edisp_hdu(cls, table_energy, table_migra, table_theta, matrix):
        ...
        hdu.header.set("TDIM7",
                             "(" + str(len(table_energy['ETRUE_LO'])) + "," + str(
                                 len(table_migra['MIGRA_LO'])) + "," + str(
